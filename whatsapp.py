...
class Whatsapp:
    ...
    def verify_sent(self, driver, messages, timeout: int = 300):
        verify_message = messages[len(messages)-1][0].split('\\r\\n')
    ...
    def send_verify_messages(self, driver, xpath, message, timeout: int = 300):
        while timeout > 0:
            try:
                inputElement = driver.find_element_by_xpath(xpath)
    ...
    def browser(self, numbers, db, verify_numbers, company):
        ...
        if(len(numbers) > 0):
            ...
            for number in numbers:
                messages = db.execQuery("SELECT text_msg FROM ocean_cfg.msg_whatsapp_log WHERE mobile_phone = '" + number[0] + "' AND date_send = '" + datetime.today().strftime('%Y%m%d') + "' AND send_ok = 0 ORDER BY task_code, user_login, sequence_msg")
                ...
                db.update("UPDATE ocean_cfg.msg_whatsapp_log SET send_ok = 1 WHERE mobile_phone = '" + number[0] + "' AND date_send = '" + datetime.today().strftime('%Y%m%d') + "'")
            ...
            for number in verify_numbers:
                ...
                self.send_verify_messages(driver, '//*[@id="main"]/footer/div[1]/div/span[2]/div/div[2]/div[1]/div/div[2]', message)
